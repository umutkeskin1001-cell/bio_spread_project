...
class FirthLogistic(BaseEstimator, ClassifierMixin):  # type: ignore[misc]
    """
    ...
    def _log_likelihood(self, beta: NDArray[Any], X: NDArray[Any], y: NDArray[Any]) -> float:
        logits = X @ beta
        p = expit(logits)
        # Standard log-likelihood with epsilon for numerical stability
        eps = 1e-15
        ll: float = float(np.sum(y * np.log(p + eps) + (1 - y) * np.log(1 - p + eps)))
    ...
    def fit(self, X: NDArray[Any], y: NDArray[Any]) -> "FirthLogistic":
        X, y = check_X_y(X, y)
