...
def test_predict_one_respects_model_window_budget():
    cfg = CassiopeiaConfig(hidden_dim=16, frp_out_dim=16, n_layers=1, max_windows=56)
    pred = predict_one(Cassiopeia(cfg), "wide", "ACGT" * 200)
...
def test_api_endpoints_with_mocked_service(tmp_path, monkeypatch):
    class MockService:
        def predict(self, sequence_id, dna):
            return {"sequence_id": sequence_id, "mobility_probs": [0.1, 0.2, 0.7],
                    "amr_probability": 0.8, "expansion_probability": 0.9,
                    "risk_score": 0.85, "top_windows": [{"window": 0.0, "weight": 0.9}]}
    ...
    with TestClient(app) as client:
        ...
        r = client.post("/predict-batch", json={
            "sequences": [{"sequence_id": "s1", "dna": "ATGCGT" * 20},
